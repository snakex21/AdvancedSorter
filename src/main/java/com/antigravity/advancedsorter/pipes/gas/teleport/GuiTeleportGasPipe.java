...
import com.antigravity.advancedsorter.network.PacketUpdateTeleportGasPipe;
...
/**
 * GUI for Teleport Gas Pipe.
 */
public class GuiTeleportGasPipe extends GuiContainer {
    ...
    public GuiTeleportGasPipe(InventoryPlayer playerInv, TileTeleportGasPipe tile) {
        super(new ContainerTeleportGasPipe(playerInv, tile));
        this.tile = tile;
        this.xSize = 350;
        this.ySize = 100;
    }
    ...
    private String getModeDisplayName(TileTeleportGasPipe.TeleportMode mode) {
        switch (mode) {
            case SEND:
                return "Send";
            case RECEIVE:
    ...
    protected void actionPerformed(GuiButton button) throws IOException {
        if (button.id == 1) {
            TileTeleportGasPipe.TeleportMode nextMode = TileTeleportGasPipe.TeleportMode
                    .values()[(tile.getMode().ordinal() + 1) % TileTeleportGasPipe.TeleportMode.values().length];
    ...
    private void sendUpdate() {
        AdvancedSorterMod.network
                .sendToServer(new PacketUpdateTeleportGasPipe(tile.getPos(), tile.getFrequency(), tile.getMode()));
    }
    ...
    protected void drawGuiContainerBackgroundLayer(float partialTicks, int mouseX, int mouseY) {
        ...
        // Panel 1 background (purple tint for gas)
        drawRect(guiLeft + 5, guiTop + 5, guiLeft + panelWidth - 5, guiTop + ySize - 5, 0xFF2D2040);
        // Panel 2 background
        drawRect(guiLeft + panelWidth + 5, guiTop + 5, guiLeft + panelWidth * 2 - 5, guiTop + ySize - 5, 0xFF2D2040);
        // Panel 3 background
        drawRect(guiLeft + panelWidth * 2 + 5, guiTop + 5, guiLeft + xSize - 5, guiTop + ySize - 5, 0xFF2D2040);
    }
    ...
    protected void drawGuiContainerForegroundLayer(int mouseX, int mouseY) {
        ...
        // Panel 1: Settings
        String title1 = "Gas Teleporter";
        fontRenderer.drawString(title1, panelWidth / 2 - fontRenderer.getStringWidth(title1) / 2, 10, 0xAA55FF);
        fontRenderer.drawString("Frequency:", 20, 25, 0xAAAAAA);
        fontRenderer.drawString("Mode:", 20, 55, 0xAAAAAA);

        // Show tank level
        int gasAmount = tile.getTank().getStored();
        int capacity = tile.getTank().getMaxGas();
        String gasName = tile.getTank().getGas() != null ? tile.getTank().getGas().getGas().getLocalizedName() : "Empty";
        fontRenderer.drawString(gasAmount + "/" + capacity + " mB", 20, 85, 0xAA55FF);
