...
import com.antigravity.advancedsorter.network.PacketUpdateTeleportPipe;
...
public class GuiTeleportPipe extends GuiContainer {
    // Use vanilla chest GUI as base - reliable and clean
    ...
    public GuiTeleportPipe(InventoryPlayer playerInv, TileTeleportPipe te) {
        super(new ContainerTeleportPipe(playerInv, te));
        this.te = te;
        this.xSize = 350;
        this.ySize = 90;
    }
    ...
    public void initGui() {
        ...
        // Center of panel 1: 350 / 6 = 58
        freqField = new GuiTextField(0, fontRenderer, guiLeft + 20, guiTop + 35, 76, 16);
        freqField.setMaxStringLength(5);
        freqField.setText(String.valueOf(te.getFrequency()));
        freqField.setFocused(true);
        freqField.setEnableBackgroundDrawing(true);

        addButton(new GuiButton(1, guiLeft + 20, guiTop + 65, 76, 20, getModeDisplayName(te.getMode())));

        // Request connection info from server
        AdvancedSorterMod.network.sendToServer(new PacketRequestTeleportInfo(te.getPos()));
    }

    private String getModeDisplayName(TileTeleportPipe.TeleportMode mode) {
        switch (mode) {
            case SEND:
                return "Send";
            case RECEIVE:
    ...
    protected void actionPerformed(GuiButton button) throws IOException {
        if (button.id == 1) {
            TileTeleportPipe.TeleportMode nextMode = TileTeleportPipe.TeleportMode.values()[(te.getMode().ordinal() + 1)
                    % TileTeleportPipe.TeleportMode.values().length];
            te.setMode(nextMode);
    ...
    private void sendUpdate() {
        AdvancedSorterMod.network
                .sendToServer(new PacketUpdateTeleportPipe(te.getPos(), te.getFrequency(), te.getMode()));
    }
    ...
    protected void drawGuiContainerForegroundLayer(int mouseX, int mouseY) {
        ...
        for (TeleportRegistry.TeleportLocation loc : connectedPipes) {
            if (loc.dimension == te.getWorld().provider.getDimension() && loc.pos.equals(te.getPos()))
                continue;
            if (te.getMode().canSend() && loc.canReceive) {
        ...
        for (TeleportRegistry.TeleportLocation loc : connectedPipes) {
            if (loc.dimension == te.getWorld().provider.getDimension() && loc.pos.equals(te.getPos()))
                continue;
            if (te.getMode().canReceive() && loc.canSend) {
                String s = getDimensionName(loc.dimension) + ": " + loc.pos.getX() + "," + loc.pos.getY() + ","
                        + loc.pos.getZ();
